...
let one = 'Tav4100BPM';
let two = 'julianLoop2';
let three = 'julianLoop3';
let four = 'julianLoop4';
let five = 'julianLoop5';
let six = 'julianLoop6';

let stepName = new Array;

for(let i = 0; i < seqRows; i++){
  stepName[i] = `julianStep${i}`;
}
...
function setup() {  // setup p5
  ...
  buttonOffColour = 'rgba(158, 119, 163, 0.6)'; // default off colours for looper buttons
  buttonOnColour = 'rgba(163, 132, 119, 0.6)'; // default on colours for looper buttons
...
function welcomeScreen() {
  ...
  text("Tavener number 4", width/2, height/10 * 2);
  textSize(cnvDimension/20);
  text( "Containing excerpts of Tavener's The Hidden Face played by Matt Maguire.", width/10, height/10, (width/10) * 8, (height/10) * 8);
